/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ReferansWhereInput } from "./ReferansWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ReferansOrderByInput } from "./ReferansOrderByInput";

@ArgsType()
class ReferansFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ReferansWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ReferansWhereInput, { nullable: true })
  @Type(() => ReferansWhereInput)
  where?: ReferansWhereInput;

  @ApiProperty({
    required: false,
    type: [ReferansOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ReferansOrderByInput], { nullable: true })
  @Type(() => ReferansOrderByInput)
  orderBy?: Array<ReferansOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ReferansFindManyArgs as ReferansFindManyArgs };
